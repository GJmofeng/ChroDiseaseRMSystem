  ...
  <div class="permission-manage-page">
    <div class="card">
      ...
      <div class="debug-info mb-4 p-4 bg-gray-100 rounded text-sm" v-if="!isCurrentUserSuperAdmin">
        ...
        <p>localStorage中的user: {{ localStorage.getItem('user') }}</p>
        <p>localStorage中的userInfo: {{ localStorage.getItem('userInfo') }}</p>
...
const isCurrentUserSuperAdmin = computed(() => {
  try {
    ...
    console.log('当前用户信息字符串:', userStr)
    
    if (userStr) {
      currentLoginUser.value = JSON.parse(userStr)
