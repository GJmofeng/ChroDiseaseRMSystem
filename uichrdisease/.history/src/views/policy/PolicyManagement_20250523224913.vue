...
const fetchPolicyList = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      // 确保data是数组
      policyList.value = Array.isArray(res.data) ? res.data : []
      if (policyList.value.length === 0) {
        ElMessage.info('暂无政策数据')
      }
    } else {
...
const handleSearch = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      // 确保data是对象或数组
      if (res.data) {
        policyList.value = Array.isArray(res.data) ? res.data : [res.data]
      } else {
        policyList.value = []
        ElMessage.info('未找到该年度的政策数据')
      }
    } else {
