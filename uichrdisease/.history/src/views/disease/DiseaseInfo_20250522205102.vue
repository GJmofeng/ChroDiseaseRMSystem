...
const getTableData = async () => {
  try {
    ...
    // 如果代码执行到这里，说明后端返回的code是0
    if (res.data && res.data.data) { // 确保data字段存在
        tableData.value = res.data.data.list
        total.value = res.data.data.total
    } else {
        // 理论上如果code是0，data应该不会是null，但以防万一
        ElMessage.error('获取数据失败，后端返回数据结构异常')
        tableData.value = []
        total.value = 0
        console.error('后端返回数据结构异常:', res.data)
    }
