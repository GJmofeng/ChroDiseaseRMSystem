  ...
  <div class="disease-info-container">
    <el-card class="box-card">
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="disease_name" label="疾病名称" width="180" />
        <el-table-column prop="disease_type" label="疾病类型" width="120" />
      ...
      >
        ...
        <el-form-item label="疾病类型" prop="diseaseType">
          <el-select v-model="form.diseaseType" placeholder="请选择疾病类型">
            ...
            <el-option label="特殊病" value="special" />
          </el-select>
        </el-form-item>
...
const getTableData = async () => {
  try {
    ...
    // 确保后端返回的code是成功状态（假设code 0 为成功）
    if (res.data && res.data.code === 0) {
      tableData.value = res.data.data.list
      total.value = res.data.data.total
    } else {
      // 如果后端返回的code不是0，也认为是获取失败，并显示后端返回的消息
      ElMessage.error(res.data.message || '获取数据失败，后端返回非成功状态')
      // 清空数据
      tableData.value = []
      total.value = 0
      console.error('后端返回非成功状态:', res.data)
    }
  } catch (error) {
    // 打印详细错误信息到控制台
    console.error('获取数据失败 Catch:', error)
    ElMessage.error('获取数据失败，请查看控制台了解详情')
    // 清空数据
    tableData.value = []
    total.value = 0
  }
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该慢病信息吗？', '提示', {
